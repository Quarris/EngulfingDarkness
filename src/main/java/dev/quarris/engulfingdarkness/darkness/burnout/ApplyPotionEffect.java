...
public class ApplyPotionEffect extends BurnoutEffect<ApplyPotionEffect.Serializer> {
    ...
    public ApplyPotionEffect(Holder<MobEffect> effect, int duration, int amplifier, boolean hidden) {
        this.effect = effect;
    ...
    public static class Serializer extends BurnoutEffect.Serializer<ApplyPotionEffect> {
        ...
        public JsonObject serialize(ApplyPotionEffect effect) {
            ...
            json.addProperty("effect", effect.effect.getRegisteredName().toString());
        ...
        public ApplyPotionEffect deserialize(JsonObject json) {
            var mobEffect = ForgeRegistries.MOB_EFFECTS.getHolder(ResourceLocation.parse(json.get("effect").getAsString())).orElse(null);
